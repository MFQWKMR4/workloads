...
pub(crate) fn run(step: &Step) -> Result<(), Box<dyn Error>> {
    ...
    let source = resolve_source(step, Path::new("runtimes/python/main.py"), "py")?;

    let count_label = count
        .map(|value| value.to_string())
        .unwrap_or_else(|| "none".to_string());
    println!("python: processes={} count={}", processes, count_label);
    ...
    for _ in 0..processes {
        ...
        let cmd_display = if let Some(value) = count {
            format!("python3 {} --count {}", source.path.display(), value)
        } else {
